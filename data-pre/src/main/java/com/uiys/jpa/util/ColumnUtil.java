 ...
 *
 * @see ColumnUtil#test(java.lang.String[]) 使用示例
 */
public class ColumnUtil {
	...
	public static <T> String getFieldName(SFunction<T, ?> fn, String split, Integer toType) {
		...
		Field field;
		try {
			field = Class.forName(serializedLambda.getImplClass()
				.replace("/", "."))
		...
		if (tableField != null && tableField.value()
		  .length() > 0) {
	...
	private static <T> SerializedLambda getSerializedLambda(SFunction<T, ?> fn) {
		...
		try {
			writeReplaceMethod = fn.getClass()
